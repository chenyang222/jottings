 ...
 * 将 "好长一段字，罗哩罗嗦的" 变成 "好长..."
 */
String.prototype.fixLen = function (len, tail) {
    var value = this;
    if (value.size() > len) {
        return value.substring(0, len) + (tail === undefined ? "..." : tail);
    } else {
...
String.prototype.format = function () {
    var args = arguments;
    var templatestr = args[0];
    return templatestr.replace(new RegExp("\\{(\\d+)\\}", "gm"), function (m, i) {
        return args[++i];
    });
}
...
Array.prototype.getCurrent = function (index) {
    for (var i = 0; i < this.length; i++) {
        if (i == index) {
            return this[i];
        }
...
Array.prototype.limit = function (start, end) {
    var data = [];
